...
parameter_types! {
    ...
    pub const MonthlyPublicFeeQuota: Balance = 100_000_000_000_000; // 100 COS
...
fn new_test_ext() -> sp_io::TestExternalities {
    ...
    pallet_balances::GenesisConfig::<Test> {
        balances: vec![
            (1, 10_000_000_000_000_000u128), // 10000 COS for testing
...
//         let replicas: u32 = 3;
//         let price: Balance = 10_000_000_000_000; // 10 COS
...
//         let amount: Balance = 50_000_000_000_000; // 50 COS

//         // 给pool充值
//         let pool = IpfsPoolAccount::get();
//         let _ = <Test as crate::Config>::Currency::deposit_creating(&pool, 1_000_000_000_000_000);

//         // 设置配额已用95 COS（剩余5 COS，不足50）
...
//         assert_eq!(used, amount); // 重置后只用了50 COS
...
fn four_layer_charge_from_ipfs_pool() {
    ...
    new_test_ext().execute_with(|| {
        ...
        let amount: Balance = 10_000_000_000_000; // 10 COS
...
fn four_layer_charge_fallback_to_subject_funding() {
    ...
    new_test_ext().execute_with(|| {
        ...
        // IpfsPool余额不足
        let pool = IpfsPoolAccount::get();
        let _ = <Test as crate::Config>::Currency::deposit_creating(&pool, 1_000_000_000); // 只有1 COS
