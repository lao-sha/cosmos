...
买家 David 消费 1000 NXS：
推荐链：Alice(10直推) → Bob(5直推) → Carol(1直推) → Eve(0直推) → David

返佣计算（叠加，受上限约束）：
├── 直推奖励: Eve 获得 1000 × 5% = 50 NXS
├── 多级分销:
│   ├── Eve (L1, 无条件): 1000 × 5% = 50 NXS
│   ├── Carol (L2, 1直推✓): 1000 × 3% = 30 NXS
│   ├── Bob (L3, 3直推✓): 1000 × 2% = 20 NXS
│   ├── Alice (L4, 5直推✓): 1000 × 1% = 10 NXS
│   └── (L5 无人或未激活)
└── 总多级分销: 110 NXS（在15%上限=150 NXS内）

总返佣：160 NXS（受限于可用返佣池）
```
