...
│        X-Costik-Signature: {Ed25519 签名}                   │
│        X-Costik-Owner: {owner_address}                      │
│        Body: {payload}                                      │
│    收集响应，记录成功/失败                                     │
└─────────────────┬──────────────────────────────────────────┘
                  ▼
...
```rust
// pallets/costik/bot-consensus/src/lib.rs
...
docker run -d \
  ...
  -e CHAIN_RPC="wss://rpc.costik.network" \
  -e OWNER_MNEMONIC="word1 word2 ... word12" \
  -p 8443:8443 \
  -v costik-data:/data \
  ghcr.io/costik/local-agent:latest
...
│  Content-Type: application/json                                      │
│  X-Costik-Consensus-Proof: {多节点签名聚合}                           │
│                                                                      │
│  {                                                                   │
...
```rust
// pallets/costik/bot-group-mgmt/src/lib.rs
...
T₂  │ Alice 点击 [🔗 绑定钱包]
    ...
    │ 节点共识: sendMessage("请访问 https://app.costik.io/bind?tg=123")
    │
...
│      │                                                               │
│      ├── N2 向 Agent 发送管理指令                                      │
│      │   Header 附加: X-Costik-Leader-Takeover: true                 │
│      │                X-Costik-Original-Leader: N7                   │
│      │                X-Costik-Takeover-Rank: 1                      │
│      │                                                               │
│      ├── Agent 验证:                                                  │
...
│  X-Costik-Node-Id: N7                                                │
│  X-Costik-Node-Signature: {...}                                      │
│  X-Costik-Leader-Takeover: false                                     │
│                                                                      │
│  Agent 验证:                                                          │
│                                                                      │
│  ┌──────────────────────────────────────────────────────────────┐   │
...
│  │       ✅ 是 Leader → 执行                                    │   │
│  │                                                              │   │
│  │   elif X-Costik-Leader-Takeover == true:                      │   │
│  │       // 是 Backup 接替                                       │   │
...
```rust
// pallets/costik/bot-consensus/src/lib.rs (扩展)
...
costik-agent/              # Sprint 2: Local Agent (独立仓库)
├── src/
│   ├── main.rs
...
costik-node/               # Sprint 3: 项目节点 (独立仓库)
├── src/
│   ├── main.rs
...
│  ──────────────────────────────────────────                          │
│  Step 1.1: 创建 pallets/costik/bot-consensus/ crate                 │
...
│  ──────────────────────────────────────────                          │
│  Step 1.7: 创建 pallets/costik/bot-registry/ crate                  │
│    │                                                                 │
│    │  定义数据结构:                                                   │
...
│  ──────────────────────────────────────────                          │
│  Step 2.1: 初始化 costik-agent 仓库                                  │
│    • cargo init costik-agent                                         │
