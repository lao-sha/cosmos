...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn delete_chart(
            ...
            // 2. 返还押金（如果有押金记录）
            let (refund, treasury) = Self::unreserve_storage_deposit(&who, chart_id)
        ...
        BlockNumberFor<T>: From<u32> + sp_runtime::traits::Saturating + PartialOrd + Copy,
    {
        ...
        pub fn estimate_deposit(privacy_mode: u8) -> Option<BalanceOf<T>> {
            ...
            // 紫微斗数命盘数据估算大小: 1000 bytes（最复杂的占卜类型之一）
            let data_size = pallet_divination_common::deposit::estimate_data_size(3, mode); // 3 = Ziwei
