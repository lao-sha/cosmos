...
fn register_bot_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(BotRegistry::register_bot(
			...
			bh, ch, salt(1), Platform::Telegram, pk,
		));
		...
		assert_eq!(bot.hash_salt, salt(1));
...
fn register_bot_fails_duplicate() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(BotRegistry::register_bot(
			RuntimeOrigin::signed(1),
			bh, community_hash(1), salt(1), Platform::Telegram, pubkey(1),
		));

		assert_noop!(
			BotRegistry::register_bot(
				RuntimeOrigin::signed(2),
				bh, community_hash(2), salt(2), Platform::Discord, pubkey(2),
			),
...
fn update_public_key_fails_same_key() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(BotRegistry::register_bot(
			RuntimeOrigin::signed(1),
			bh, community_hash(1), salt(1), Platform::Telegram, pk,
		));
...
fn helper_functions_work() {
	new_test_ext().execute_with(|| {
		...
		// get_bot_salt
		assert_eq!(BotRegistry::get_bot_salt(&bh), Some(salt(1)));
		assert_eq!(BotRegistry::get_bot_salt(&bot_hash(99)), None);
	});
}
