# Nexus · Telegram Bot 混合代理架构设计（方案 C）

> 群主持有 Token × 轻量本地代理 × 项目方路由网关 × 多后端验证 × Nexus 链上授权
>
...
| **链上授权** | 群主的服务资格、公钥、IP 白名单均由 Nexus pallet 管理 |
...
┌─────────────────────────────────────────────────┐
│ ④ HTTPS POST 到项目方路由网关                      │
│    POST https://router.nexus.network/v1/ingest  │
└─────────────────────────────────────────────────┘
```
...
```
POST https://backend-a.example.com/nexus/incoming
Content-Type: application/json
X-Nexus-Owner-Signature: {群主原始 Ed25519 签名 (透传)}
X-Nexus-Gateway-Signature: HMAC-SHA256({backend_a_secret}, body)
X-Nexus-Gateway-Id: gw-001
...
**双重签名设计**:
- `X-Nexus-Owner-Signature` — 群主 Ed25519 签名（后端可用链上公钥独立验证）
- `X-Nexus-Gateway-Signature` — 路由网关 HMAC 签名（后端确认消息经过路由网关）
...
```rust
// pallets/nexus/bot-registry/src/lib.rs
...
```rust
// pallets/nexus/bot-auth/src/lib.rs
...
```rust
// pallets/nexus/bot-billing/src/lib.rs
...
│     docker run -e BOT_TOKEN="123456:ABC-DEF" nexus-agent       │
│       │                                                         │
│       ▼                                                         │
│  ③ Local Agent 启动时：                                          │
│     - Token 加载到进程内存                                       │
...
T₃: 并发扇出到 N 个后端 (< 50ms)
    │
    ├──▶ Backend A: POST /nexus/incoming  →  独立 5 层验证  →  业务逻辑
    ├──▶ Backend B: POST /nexus/incoming  →  独立 5 层验证  →  业务逻辑
    └──▶ Backend C: POST /nexus/incoming  →  独立 5 层验证  →  业务逻辑
    │
    ▼
T₄: 代理网关收集投递结果 (< 200ms)
    │
...
| **链上授权** | Nexus pallet 管理服务生命周期，Merkle 证明支持离线验证 |
