...
| v0.4.0 | 2026-01-18 | 买家押金机制（简化版统一比例）、自动过期处理、存储膨胀防护、争议功能迁移至 pallet-arbitration |
...
|----------|----------|
| 首购用户 | 免押金 |
| 其他用户 | 统一比例（默认 10%，由 `DepositRate` 配置） |

**押金处理规则**：
- 订单完成：100% 退还买家
- 买家主动取消：30% 没收给做市商，70% 退还（由 `CancelPenaltyRate` 配置）
- 订单超时：100% 没收给做市商
- 争议买家胜诉：100% 退还买家
- 争议做市商胜诉：100% 没收给做市商
...
|------|--------|------|
| `MinDeposit` | - | 最小押金金额 |
| `DepositRate` | 1000（10%） | 统一押金比例（首购用户免押金） |
...
2. **买家押金**：统一比例押金机制（首购免押金），保护做市商权益
3. **TRON 交易哈希防重放**：记录已使用的交易哈希，防止重复提交
4. **做市商押金验证**：创建订单时验证做市商押金 USD 价值（最低 800 USD）
5. **证据 CID 锁定**：争议期间自动锁定证据 CID，防止删除
6. **自动过期处理**：链上自动处理超时订单，避免资金长期锁定
7. **买家额度管理**：通过 `pallet-trading-credit` 进行额度检查和占用
