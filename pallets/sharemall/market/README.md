# 店铺代币交易市场模块 (pallet-sharemall-market)
...
```rust
// 启用 COS 和 USDT 交易
SharemallMarket::configure_market(
...
SharemallMarket::place_sell_order(
    Origin::signed(alice),
    shop_id,
    1000,           // token_amount
...
SharemallMarket::place_usdt_sell_order(
    Origin::signed(alice),
    shop_id,
    1000,                           // token_amount
...
SharemallMarket::take_usdt_sell_order(
    Origin::signed(bob),
    order_id,
    None,                                           // amount (全部)
...
SharemallMarket::accept_usdt_buy_order(
    Origin::signed(alice),
    order_id,
    None,                           // amount (全部)
    b"TXyz...".to_vec(),           // tron_address
...
// Bob 链下转 USDT 后确认支付
SharemallMarket::confirm_usdt_payment(
    Origin::signed(bob),
    trade_id,
...
```rust
// 获取订单簿深度（每边 10 档）
let depth = SharemallMarket::get_order_book_depth(shop_id, 10);
// depth.asks: 卖盘（按价格升序）
// depth.bids: 买盘（按价格降序）
// depth.best_ask: 最优卖价
// depth.best_bid: 最优买价
...
```rust
// 获取市场摘要
let summary = SharemallMarket::get_market_summary(shop_id);
...
```rust
// 获取最优买卖价
let (best_ask, best_bid) = SharemallMarket::get_best_prices(shop_id);

// 获取买卖价差
let spread = SharemallMarket::get_spread(shop_id);
```
...
```rust
// 店主配置价格保护参数
SharemallMarket::configure_price_protection(
    Origin::signed(shop_owner),
    shop_id,
    true,   // enabled: 启用价格保护
...
```rust
// 获取 1小时 TWAP
let twap_1h = SharemallMarket::calculate_twap(shop_id, TwapPeriod::OneHour);

// 获取 24小时 TWAP
let twap_24h = SharemallMarket::calculate_twap(shop_id, TwapPeriod::OneDay);

// 获取 7天 TWAP
let twap_7d = SharemallMarket::calculate_twap(shop_id, TwapPeriod::OneWeek);
```
...
```rust
// 店主在熔断时间到期后手动解除
SharemallMarket::lift_circuit_breaker(
    Origin::signed(shop_owner),
    shop_id,
)?;
```
...
SharemallMarket::set_initial_price(
    Origin::signed(shop_owner),
    shop_id,
...
- `pallet-sharemall-common`: 公共类型和 Trait
- `pallet-sharemall-shop`: 店铺信息查询
- `pallet-sharemall-token`: 店铺代币操作
