# pallet-sharemall-shop

> 🏪 ShareMall 店铺管理模块 - 店铺生命周期管理与运营资金管理
...
`pallet-sharemall-shop` 是 ShareMall 商城系统的店铺管理模块，负责店铺的完整生命周期管理，包括创建、运营资金管理、状态管理和治理审核。
...
└─────────────────────────────────────────────────────────────────┘
         ...
         ▼                              ▼
┌─────────────────────┐    ┌─────────────────────────────────────┐
│  pallet-trading     │    │        pallet-sharemall-product     │
│     -pricing        │    │           (商品模块)                 │
│    (定价模块)        │    │  • 验证店铺存在性                    │
│  • COS/USDT 价格    │    │  • 验证店主身份                      │
└─────────────────────┘    └─────────────────────────────────────┘
...
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  地址: PalletId(*b"sm/shop/").into_sub_account(shop_id)        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```toml
[dependencies]
pallet-sharemall-shop = { path = "pallets/sharemall/shop", default-features = false }

[features]
std = [
    "pallet-sharemall-shop/std",
]
```
...
impl pallet_sharemall_shop::Config for Runtime {
...
```bash
# 运行单元测试
cargo test -p pallet-sharemall-shop

# 运行特定测试
cargo test -p pallet-sharemall-shop test_create_shop
```
...
- [pallet-sharemall-product](../product/README.md)
