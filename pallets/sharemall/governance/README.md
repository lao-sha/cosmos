# pallet-sharemall-governance

> 🏛️ ShareMall 店铺代币治理模块 - 去中心化店铺决策系统
...
`pallet-sharemall-governance` 是 ShareMall 商城系统的店铺治理模块，实现基于店铺代币的去中心化决策机制。代币持有者可以参与店铺运营决策，实现社区驱动的店铺管理。
...
└─────────────────────────────────────────────────────────────────┘
         ...
         ▼                              ▼
┌─────────────────────┐    ┌─────────────────────────────────────┐
│  pallet-sharemall   │    │       pallet-sharemall-token        │
│      -shop          │    │           (代币模块)                 │
│    (店铺模块)        │    │  • 代币余额查询                      │
│  • 店铺存在性验证    │    │  • 总供应量查询                      │
│  • 店主身份验证      │    │  • 代币启用状态                      │
...
```toml
[dependencies]
pallet-sharemall-governance = { path = "pallets/sharemall/governance", default-features = false }

[features]
std = [
    "pallet-sharemall-governance/std",
]
```
...
impl pallet_sharemall_governance::Config for Runtime {
    type RuntimeEvent = RuntimeEvent;
    type Balance = Balance;
    type ShopProvider = ShareMallShop;
    type TokenProvider = ShareMallTokenProvider;
...
```javascript
// 创建价格调整提案
api.tx.shareMallGovernance.createProposal(
    1,  // shop_id
...
**示例：**
```javascript
api.tx.shareMallGovernance.vote(1, { Yes: null })
```
...
```bash
# 运行单元测试
cargo test -p pallet-sharemall-governance

# 运行特定测试
cargo test -p pallet-sharemall-governance test_create_proposal
```
...
- [pallet-sharemall-token](../token/README.md)
