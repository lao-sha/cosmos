# pallet-sharemall-commission

> 💰 ShareMall 返佣管理模块 - 支持多选返佣模式
...
`pallet-sharemall-commission` 是 ShareMall 商城系统的返佣管理模块，支持**多选返佣模式**，店铺可同时启用多种返佣方式，返佣按顺序叠加计算。
...
```toml
[dependencies]
pallet-sharemall-commission = { path = "pallets/sharemall/commission", default-features = false }

[features]
std = [
    "pallet-sharemall-commission/std",
]
```
...
impl pallet_sharemall_commission::Config for Runtime {
    type RuntimeEvent = RuntimeEvent;
    type Currency = Balances;
    type ShopProvider = ShareMallShop;
    type MemberProvider = ShareMallMember;
...
ShareMallCommission::set_custom_level_diff_config(origin, shop_id, level_rates.try_into().unwrap(), 10)?;
```
...
ShareMallCommission::set_withdrawal_config(
    origin,
    shop_id,
...
```rust
ShareMallCommission::use_shopping_balance(origin, shop_id, amount)?;
```
