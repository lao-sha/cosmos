...
impl LeaderExecutor {
    ...
    pub async fn execute_as_leader(
        ...
        if matches!(action_type, ActionType::NoAction) {
            debug!(msg_id, "无需执行动作");
...
mod tests {
    ...
    fn test_determine_action_join_request() {
        ...
        assert!(matches!(action_type, ActionType::ApproveChatJoinRequest));
    ...
    fn test_determine_action_ban_command() {
        ...
        assert!(matches!(action_type, ActionType::BanUser));
    ...
    fn test_determine_action_plain_message() {
        ...
        assert!(matches!(action_type, ActionType::NoAction));
    }
}
