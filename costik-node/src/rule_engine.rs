...
use crate::leader::{ActionType, ExecuteAction};
...
pub struct RuleContext {
    ...
    /// 群规则（从链上缓存）
    pub group_rules: Option<CachedGroupRules>,
}

/// 缓存的群规则
#[derive(Debug, Clone)]
pub struct CachedGroupRules {
...
impl RuleEngine {
    ...
    /// 评估消息 → 返回动作
    pub fn evaluate(&self, message: &SignedMessage) -> RuleAction {
    ...
    fn build_context(message: &SignedMessage) -> RuleContext {
        ...
        RuleContext {
            ...
            is_callback,
            callback_data,
            group_rules: None, // TODO: 从 chain_cache 获取
        }
    }
}
...
impl Rule for JoinRequestRule {
    ...
    fn evaluate(&self, ctx: &RuleContext) -> Option<RuleAction> {
        ...
        // 根据群规则决定是否自动通过
        let policy = ctx.group_rules.as_ref()
            .map(|r| r.join_policy.as_str())
            .unwrap_or("AutoApprove");

        match policy {
            "ManualApproval" => None, // 不自动处理
            _ => Some(RuleAction {
                action_type: ActionType::ApproveChatJoinRequest,
                chat_id: ctx.chat_id,
    ...
    fn evaluate(&self, ctx: &RuleContext) -> Option<RuleAction> {
        ...
        match cmd {
            "ban" | "kick" => {
                ...
                Some(RuleAction {
                    action_type: ActionType::BanUser,
                    chat_id: ctx.chat_id,
                    params: serde_json::json!({ "user_id": user_id }),
            ...
            "mute" => {
                ...
                let duration = ctx.command_args.as_deref()
                    ...
                    .unwrap_or(3600);
                Some(RuleAction {
                    action_type: ActionType::MuteUser,
                    chat_id: ctx.chat_id,
                    params: serde_json::json!({
                        "user_id": user_id,
            ...
            "unmute" => {
                ...
                if user_id == 0 { return None; }
                Some(RuleAction {
                    action_type: ActionType::UnmuteUser,
                    chat_id: ctx.chat_id,
                    params: serde_json::json!({ "user_id": user_id }),
            ...
            "pin" => {
                ...
                Some(RuleAction {
                    action_type: ActionType::PinMessage,
                    chat_id: ctx.chat_id,
...
impl Rule for LinkFilterRule {
    ...
    fn evaluate(&self, ctx: &RuleContext) -> Option<RuleAction> {
        let filter_enabled = ctx.group_rules.as_ref()
            .map(|r| r.filter_links)
            .unwrap_or(false);
...
mod tests {
    ...
    fn test_join_request_auto_approve() {
        ...
        let action = engine.evaluate(&msg);
        assert!(matches!(action.action_type, ActionType::ApproveChatJoinRequest));
    ...
    fn test_ban_command() {
        ...
        let action = engine.evaluate(&msg);
        assert!(matches!(action.action_type, ActionType::BanUser));
    ...
    fn test_mute_with_duration() {
        ...
        let action = engine.evaluate(&msg);
        assert!(matches!(action.action_type, ActionType::MuteUser));
