/**
 * 星尘玄鉴 - 密钥存储（Web 版本）
 * 使用 localStorage 替代 expo-secure-store
 */
...
export async function initializeCrypto(): Promise<void> {
  console.log('[Keystore] Crypto initialized (web - localStorage)');
}
...
export async function loadAllKeystores(): Promise<LocalKeystore[]> {
  try {
    const data = localStorage.getItem(STORAGE_KEYS.KEYSTORES);
    if (!data) return [];
...
async function saveAllKeystores(keystores: LocalKeystore[]): Promise<void> {
  localStorage.setItem(STORAGE_KEYS.KEYSTORES, JSON.stringify(keystores));
}

export async function getCurrentAddress(): Promise<string | null> {
  try { return localStorage.getItem(STORAGE_KEYS.CURRENT_ADDRESS); }
  catch { return null; }
}

export async function setCurrentAddress(address: string): Promise<void> {
  localStorage.setItem(STORAGE_KEYS.CURRENT_ADDRESS, address);
}
...
export async function storeEncryptedMnemonic(
  ...
  console.log('[Keystore] Mnemonic stored securely (web)');
}
...
export async function deleteWalletByAddress(address: string): Promise<void> {
  ...
  if (currentAddr === address) {
    if (filtered.length > 0 && filtered[0]) await setCurrentAddress(filtered[0].address);
    else localStorage.removeItem(STORAGE_KEYS.CURRENT_ADDRESS);
  }
}
...
export async function deleteAllWallets(): Promise<void> {
  localStorage.removeItem(STORAGE_KEYS.KEYSTORES);
  localStorage.removeItem(STORAGE_KEYS.CURRENT_ADDRESS);
